...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for i in range(self.width):
            for j in range(self.height):
                pos = Point(i,j)
                self.block_list[(i,j)] = Block(pos, color)
    ...
    def get_block_neighbors(self, block):
        ...
        for coord in coords:
            neighbor_coord = (block_coords[0] + coord[0],block_coords[1] + coord[1])
            if (neighbor_coord[0] >= 0) and (neighbor_coord[0] < 12) and (neighbor_coord[1] >= 0) and (neighbor_coord[1] < 12):
                neighbors.append(self.block_list[neighbor_coord])
        return neighbors
    ...
    def simulate(self):
        ...
        for block in self.block_list.values():
            ...
            count = self.number_of_live_blocks(neighbors)
            if block.is_live():
    ...
    def number_of_live_blocks(self,block_list):
        count = 0
